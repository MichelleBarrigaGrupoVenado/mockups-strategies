...
export function ConditionRow({ condition, onChange, onRemove }: ConditionRowProps) {
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-card px-3 py-2.5">
      ...
      <NativeSelect className="min-w-36 flex-1 basis-40" value={condition.field} onChange={(e) => onChange({ field: e.target.value })}>
      ...
      <Input className="w-24 min-w-0 flex-1 sm:w-28 sm:flex-none" value={condition.value} onChange={(e) => onChange({ value: e.target.value })} />
